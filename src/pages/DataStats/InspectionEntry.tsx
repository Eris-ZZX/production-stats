...
import { mockStations, mockDefects, mockInspectionRecords } from '../../mockData';
...
export default function InspectionEntry() {
  ...
  const [records, setRecords] = useState<InspRecord[]>(mockInspectionRecords);
  const [editingKey, setEditingKey] = useState<number | null>(null);
  const [adding, setAdding] = useState(false);

  const stations = mockStations.filter(s => s.isActive);
  ...
  function syncGlobal(u: InspRecord[]) { mockInspectionRecords.length = 0; mockInspectionRecords.push(...u); }
  ...
  const saveEdit = async (id: number) => {
    ...
    const u = records.map(r => r.id === id ? { ...r, productSn: row.productSn, minorSection: row.minorSection || '-', majorSection: row.majorSection || '-', productionDefects: row.productionDefects || [], fqcDefects: row.fqcDefects || [], inspectionDate: row.inspectionDate.format('YYYY-MM-DD') } : r);
    setRecords(u); syncGlobal(u); cancel(); message.success('已保存');
  };
  ...
  const saveNew = async () => {
    ...
    const u = [...records, { id: maxId + 1, productId: currentProduct?.id || 1, productSn: row.productSn, minorSection: row.minorSection || '-', majorSection: row.majorSection || '-', productionDefects: row.productionDefects || [], fqcDefects: row.fqcDefects || [], inspectionDate: row.inspectionDate.format('YYYY-MM-DD'), createdAt: '' }];
    setRecords(u); syncGlobal(u); cancel(); message.success('已新增');
  };

  const add = () => { setAdding(true); setEditingKey(null); editForm.resetFields(); };

  const del = (id: number) => { const u = records.filter(r => r.id !== id); setRecords(u); syncGlobal(u); message.success('已删除'); };
  ...
  const handleSingleAdd = () => {
    ...
    const maxId = records.reduce((max, r) => Math.max(max, r.id), 0);
    const u = [...records, { id: maxId + 1, productId: currentProduct?.id || 1, productSn: values.productSn, minorSection: values.minorSection || '-', majorSection: values.majorSection || '-', productionDefects: values.productionDefects || [], fqcDefects: values.fqcDefects || [], inspectionDate: values.inspectionDate.format('YYYY-MM-DD'), createdAt: '' }];
    setRecords(u); syncGlobal(u);
        ...
        <Form form={form} layout="inline" style={{ flexWrap: 'wrap', gap: 8 }}>
          ...
          <Form.Item name="productionDefects" label="生产外检缺陷">
            <Select mode="multiple" allowClear style={{ width: 280 }} placeholder="可多选"
              options={defects.map(d => ({ value: d.defectCode, label: `${d.component} / ${d.type} / ${d.location} / ${d.defect}` }))} />
          </Form.Item>
      ...
      <Card title={<span>已录入记录 ({filteredInspRecords.length} 条) <SmartFilterBar fields={inspFilterFields} searchText={inspSearch} onSearchChange={setInspSearch} conditions={inspConditions} onConditionsChange={setInspConditions} /></span>}>
        <Form form={editForm} component={false}>
          <Table
            ...
            columns={getColumns() as any} pagination={{ pageSize: 10 }} size="small" />
        </Form>
      </Card>
    </div>
  );
