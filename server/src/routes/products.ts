...
import { requireAdmin, requireAnyAuth, requireConfigAuth, verifyToken } from '../auth.js';
...
router.get('/:id', requireAdmin, (req, res) => {
  ...
  const r = row as any;
  res.json({ id: r.id, name: r.name, isActive: !!r.is_active,
    pwdRead: r.pwd_read ? '••••••' : '', pwdEntry: r.pwd_entry ? '••••••' : '', pwdConfig: r.pwd_config ? '••••••' : '' });
});
...
router.post('/', requireAdmin, (req, res) => {
  ...
  db.prepare('INSERT INTO product_defect_codes (product_line_id, defect_code, component, type, location, defect, is_active) SELECT ?, defect_code, component, type, location, defect, is_active FROM defect_codes').run(newId);
...
router.put('/:id', requireAdmin, (req, res) => {
  ...
  // Only update password if a real new value is provided (not mask, not empty)
  if (pwdRead && pwdRead !== '••••••') { updates.push('pwd_read = ?'); params.push(hashPassword(pwdRead)); }
  if (pwdEntry && pwdEntry !== '••••••') { updates.push('pwd_entry = ?'); params.push(hashPassword(pwdEntry)); }
  if (pwdConfig && pwdConfig !== '••••••') { updates.push('pwd_config = ?'); params.push(hashPassword(pwdConfig)); }
  params.push(req.params.id);
  db.prepare(`UPDATE product_lines SET ${updates.join(', ')} WHERE id = ?`).run(...params);
  res.json({ ok: true });
});
...
// 所有品号（公开）
router.get('/skus/all', requireAnyAuth, (_req, res) => {
...
// 创建品号
router.post('/skus', requireConfigAuth, (req, res) => {
...
router.put('/skus/:id', requireConfigAuth, (req, res) => {
  const pid = (req as any).productId;
  // Product context: verify SKU belongs to current product
  if (pid) {
    const sku = db.prepare('SELECT product_line_id FROM product_skus WHERE id = ?').get(req.params.id) as any;
    if (!sku) { res.status(404).json({ error: '品号不存在' }); return; }
    if (sku.product_line_id !== pid) { res.status(403).json({ error: '无权操作其他产品的品号' }); return; }
  }
...
router.delete('/skus/:id', requireConfigAuth, (req, res) => {
  const pid = (req as any).productId;
  const sku = db.prepare('SELECT id, product_line_id FROM product_skus WHERE id = ?').get(req.params.id) as any;
  if (!sku) { res.status(404).json({ error: '品号不存在' }); return; }
