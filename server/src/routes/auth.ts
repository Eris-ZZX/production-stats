...
router.post('/admin-login', (req, res) => {
  ...
  const acc = db.prepare('SELECT * FROM admin_accounts WHERE username = ? AND password = ? AND is_active = 1').get(username, password) as any;
  if (!acc) {
...
router.post('/product-login', (req, res) => {
  ...
  if (!productId || !password) {
    res.status(400).json({ error: '请选择产品并输入密码' });
    return;
  }
  ...
  if (pl.pwd_config && password === pl.pwd_config) role = 'config';
  else if (pl.pwd_entry && password === pl.pwd_entry) role = 'entry';
  else if (pl.pwd_read && password === pl.pwd_read) role = 'read';
